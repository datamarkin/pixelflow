---
title: Filters
description: Contains 14 functions for filters
---

# Filters

Contains 14 functions for filters.
...
<ParamField path="threshold" type="float" required>
  Minimum confidence score (inclusive). Range: [0.0, 1.0]. Detections with confidence >= threshold are retained.
</ParamField>

### Returns

<ResponseField name="result" type="Detections">
  New Detections object containing only high-confidence detections.
</ResponseField>
...
<ParamField path="class_ids" type="Union[int, str, List[Union[int, str]]]" required>
  Single class ID or list of class IDs to include. Accepts both numeric IDs and string class names.
</ParamField>

### Returns

<ResponseField name="result" type="Detections">
  New Detections object containing only detections with matching class IDs.
</ResponseField>
