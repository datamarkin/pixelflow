...
def rectangle(image, top_left, bottom_right, line_color=(0, 255, 0), thickness=1):
    """
    ...
    cv2.rectangle(image, top_left, bottom_right, color=line_color, thickness=thickness)
...
def text(image, text, position, font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=1,
         text_color=(0, 255, 0), thickness=2, background_color=None):
    """
    Draw simple text on an image. Optionally, draw a background behind the text.
    ...
    if background_color is not None:
        ...
        cv2.rectangle(image, top_left, bottom_right, background_color, thickness=cv2.FILLED)

    # Draw the text directly on the image
...
def masks(image, predictions, labels=True):
    """
    Draw masks (polygons) on an image from a list of predictions.

    Parameters:
    - image: The original image (NumPy array) to draw masks on.
    - predictions: A list of prediction objects containing mask information.
    - labels: Boolean indicating whether to draw labels (currently not implemented).

    Returns:
    - The image with all masks drawn as polygons.

    Note:
    Each prediction object should have a 'masks' attribute that contains
    a list of polygon points defining the mask boundaries.
    The function uses the polygon() drawing function to render each mask.
    """
